...
базі права лежать у таблиці `role_permissions`: їх кладе туди міграція
6a3a66f4347b, а наступні міграції правлять окремі клітинки.

Щоб копії не розійшлись, є тест, який звіряє цей словник із сумою тих
міграцій (tests/test_permissions.py). Розійдуться — тест впаде.
"""
...
DEFAULT_MATRIX: dict[str, dict[str, tuple]] = {
    ...
    # Оператор на вагах: у рейсі йому належать лише маси. Нетто в переліку
    # немає — воно рахується з брутто й тари, а не вводиться.
    ROLE_OPERATOR: {
        CATEGORY_COMPANY: (0, 1, 0, 0, SCOPE_COMPANY, None),
        CATEGORY_TRIPS: (0, 1, 1, 0, SCOPE_COMPANY, "bmass,tmass"),
    },
