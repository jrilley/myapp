...
**Склад меню залежить від користувача.** Хто є в `ADMIN_TELEGRAM_IDS`, бачить
два додаткові пункти:

| Кнопка | Кому видно | Куди веде |
|---|---|---|
| 📝 Нова заявка | усім | починає анкету |
| 📋 Мої заявки | усім | свої заявки, у кожної — кнопка 🗑 |
...
> людині або згенерувати вручну, тому кожен адмінський хендлер перевіряє
> `settings.is_admin()` при виклику, а не покладається на те, що кнопки не
> було видно. Те саме стосується кнопки 🗑: право визначає
> `app/bot/actions.py`, а не наявність кнопки на екрані.
