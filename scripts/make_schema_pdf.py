...
# Перелік статусів читаємо з моделі, а не переписуємо сюди: інакше аркуш
# розійшовся б із кодом тихо, при першому ж новому статусі.
...
from app.models import TRIP_STATUSES  # noqa: E402
...
MATRIX_ROWS = [
    ...
    ("Оператор", ("R", "—", "—", "RE — брутто й тара"), "своя компанія"),
...
TRIP_STATUSES_CHAIN = [
    st if i == len(TRIP_STATUSES) - 1 else f"{st}  →"
    for i, st in enumerate(TRIP_STATUSES)
]
...
def page_trip(s: Sheet) -> None:
    ...
    # Середня й права колонки закінчуються вище за ліву — статус стає туди,
    # а не під таблицю видимості, де читався б як її п'ятий рядок.
    ty4 = s.section(xs[1], 432, col_w * 2 + gap, "статус — лише з переліку")
    s.text(xs[1], ty4 + 14, "  ".join(TRIP_STATUSES_CHAIN),
           font="Mono", size=8, color=TEAL)
    s.wrap(xs[1], ty4 + 34, col_w * 2 + gap,
           "Обирається кнопкою; у callback_data їде номер, а не назва — "
    ...
    s.text(M, s.H - 22,
           ...
           "(company.company_chat_id); кожна наступна правка перемальовує те "
           "саме повідомлення. Збій розсилки рейсу не скасовує.",
           font="Mono", size=7, color=SLATE)
